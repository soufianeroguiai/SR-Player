...
class _HomeScreenState extends State<HomeScreen> {
  int _navIndex = 0;         // 0=الكل  1=الأخيرة  2=المجلدات
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: _PillNavBar(
        ...
        items: const [
          _NavItem(icon: Symbols.video_library_rounded,    label: 'الكل'),
          _NavItem(icon: Symbols.history_rounded,          label: 'الأخيرة'),
          _NavItem(icon: Symbols.folder_rounded,           label: 'المجلدات'),
        ],
      ),
    );
  }
...
// ════════════════════════════════════════════════
//  شريط التنقل بالـ Pill style – مطابق للصورة
// ════════════════════════════════════════════════
class _NavItem {
  final IconData icon;
  final String label;
...
class _PillNavBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      // الخلفية: كبسولة معلّقة فوق حافة الشاشة مثل الصورة
                    ...
                    children: [
                      AnimatedSwitcher(
                        ...
                        child: Icon(
                          ...
                          size: selected ? 26 : 24,
                          fill: selected ? 1 : 0, // Material Symbols filled/outlined
                        ),
                      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      if (folders.isNotEmpty)
        _Chips(folders: folders, selected: selectedFolder, onChanged: onFolderChanged),
      ...
      itemBuilder: (_, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text('${videos.length} فيديو  •  $size',
...
class _SearchDelegate extends SearchDelegate<VideoItem?> {
  ...
  Widget _list(BuildContext context) {
    ...
    if (results.isEmpty) {
      return Center(child: Text('ما لقينا نتائج', style: TextStyle(color: cs.onSurfaceVariant)));
    }
    return ListView.builder(
